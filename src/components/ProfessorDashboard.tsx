...
export function ProfessorDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'courses' | 'assessments' | 'grading'>('courses')
  ...
  const fetchProfessorData = async () => {
    try {
      ...
      const coursesResponse = await fetch('/api/course-instances?professor=true')
  ...
  const fetchAssessments = async (courseInstanceId: string) => {
    try {
      const response = await fetch(`/api/assessments?courseInstance=${courseInstanceId}`)
  ...
  const handleScoreSubmit = async (scoreId: string, value: number, feedback: string) => {
    try {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Manage your courses, assessments, and grades</p>
      </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium">
                        {new Date(assessment.submissionWindow.opensAt).toLocaleDateString()} -
                        {new Date(assessment.submissionWindow.closesAt).toLocaleDateString()}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-600">Grading Window:</span>
                      <p className="font-medium">
                        {new Date(assessment.gradingWindow.opensAt).toLocaleDateString()} -
                        {new Date(assessment.gradingWindow.closesAt).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                </CardContent>
          ...
          <div className="space-y-4">
            {studentScores.map((score) => (
              <Card key={score.id} className="hover:shadow-lg transition-shadow">
                <CardHeader>
                  <CardTitle className="text-lg">{score.student.name}</CardTitle>
                  <p className="text-sm text-gray-600">Student ID: {score.student.studentId}</p>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        />
                        <p className="text-sm text-gray-600">Max: {score.maxValue}</p>
                      </div>
                      <div>
