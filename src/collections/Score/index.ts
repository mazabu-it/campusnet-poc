...
export const Score: CollectionConfig = {
  ...
  access: {
    admin: authenticated,
    create: multiRoleAccess(superAdminAccess, adminAccess, professorAccess, assistantAccess),
    ...
    update: multiRoleAccess(superAdminAccess, adminAccess, professorAccess, assistantAccess),
  },
  admin: {
    defaultColumns: ['assessment', 'student', 'value', 'gradedBy', 'gradedAt'],
  ...
  fields: [
    ...
    },
    {
      ...
      type: 'number',
      admin: {
        description: 'Maximum possible score (from assessment template)',
      },
    },
    {
      name: 'percentage',
      ...
      ({ data }) => {
        ...
        if (data.value && data.maxValue) {
          ...
          if (data.isLate && data.latePenaltyApplied > 0) {
            data.finalValue = data.value * (1 - data.latePenaltyApplied / 100)
          } else {
