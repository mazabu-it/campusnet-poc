...
export const calculateGradeEndpoint: Endpoint = {
  ...
  handler: async (req) => {
    try {
      ...
      if (!enrollmentId) {
        return Response.json({ error: 'Enrollment ID is required' }, { status: 400 })
      }
      ...
      return Response.json({ error: 'Failed to calculate grade' }, { status: 500 })
    }
  },
}
...
export const updateGradeAggregateEndpoint: Endpoint = {
  ...
  handler: async (req) => {
    try {
      ...
      return Response.json({ error: 'Failed to update grade aggregate' }, { status: 500 })
    }
  },
}
...
export const calculateStudentGPAEndpoint: Endpoint = {
  ...
  handler: async (req) => {
    try {
      ...
      return Response.json({ error: 'Failed to calculate GPA' }, { status: 500 })
    }
  },
}
...
export const enrollStudentEndpoint: Endpoint = {
  ...
  handler: async (req) => {
    try {
      const { studentId, courseInstanceId, enrollmentType = 'required' } = await req.json()
...
export const submitScoreEndpoint: Endpoint = {
  ...
  handler: async (req) => {
    try {
      const { assessmentId, studentId, value, feedback, notes } = await req.json()
      ...
      if (assessment.status !== 'open') {
        return Response.json({ error: 'Assessment is not open for scoring' }, { status: 400 })
      }
